...
<div class="page-heading">
            ...
            <div class="col-12 col-md-6 order-md-2 order-first">
                <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page"> {{ __('local.Add New User') }}</li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <div class="card-header">
        <h4 class="card-title">{{ __('local.User Information') }}</h4>
    </div>
    <div class="card-body">
        ...
        <form action="{{ route('user.store') }}" method="POST" class="form" id="create-user" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                <div class="btn-group" role="group" aria-label="User Type Selection">
                    ...
                    <label class="btn btn-outline-primary" for="representative">{{ __('local.Representative') }}</label>
                </div>
                @error('user_type')
            ...
            <div class="form-group">
                <label for="email">{{ __('local.Email') }}</label>
                <input type="email" class="form-control @error('email') is-invalid @enderror" name="email" value="{{ old('email') }}" id = "email" required placeholder="{{ __('local.Email') }}">
                @error('email')
                <div class="invalid-feedback">
                    <i class="bx bx-radio-circle"></i> {{ $message }}
                </div>
            ...
            <div class="tab-content mt-3">
                ...
                <div id="supervisor-section" class="user-section @if(old('user_type') != App\Enums\UserTypeEnum::SUPERVISOR->value) d-none @endif">                    <label for="city_id">{{ __('local.City') }}</label>
                    <fieldset class="form-group">
                        <select name="city_id" class="form-select @error('city_id') is-invalid @enderror" id="city_id" @if(old('user_type') == App\Enums\UserTypeEnum::SUPERVISOR->value) required @endif>
                            <option disabled selected hidden>{{ __('local.Select City') }}</option>
